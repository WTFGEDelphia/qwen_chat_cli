...
async def test_stateful_stream_chat_serializes_complete_request(monkeypatch):
    ...
    async def fake_stream_chat_once(self, run_mode, prompt, model):
        nonlocal active
        active += 1
    ...
    async def collect(prompt: str):
        return [
            chunk
            async for chunk in client.stream_chat("stateful", prompt, "qwen3.6-plus")
        ]
...
@pytest.mark.asyncio
async def test_login_reads_token_from_response_cookies():
    """验证 login 从响应 cookies 中读取 token（httpx 传输层自动合并 cookie jar）"""
    client = AsyncQwenClient("test@example.com", "password")

    mock_response = AsyncMock()
    mock_response.status_code = 200
    mock_response.cookies = {"token": "session_token_123"}
